...
local options = {
	...
	args = {
		...
		stackingHelper = {
			type = "group",
			order = 300,
			name = "Stacking Helper",
			desc = "Makes it easier to see how close you are to target-marked players",
			get = function(info)
				return db.stackingHelper[info[#info]]
			end,
			set = function(info, v)
				db.stackingHelper[info[#info]] = v
				toggleOverride = nil
			end,
			args = {
				enabled = {
					name = "Enable stacking helper",
					type = "toggle",
					order = 1,
				},
				allTargetMarkers = {
					name = "All target markers",
					type = "toggle",
					order = 2,
					disabled = function() return not db.stackingHelper.enabled end
				},
				star = {
					name = "star",
					type = "toggle",
					disabled = function()
						return db.stackingHelper.allTargetMarkers or (not db.stackingHelper.enabled) 
					end
				},
				circle = {
					name = "circle",
					type = "toggle",
					disabled = function() return db.stackingHelper.allTargetMarkers or (not db.stackingHelper.enabled) end
				},
				diamond = {
					name = "diamond",
					type = "toggle",
...
-- Make the table for target markers
local targetMarkerTable = {
	"star",
	"circle",
	"diamond",
	"triangle",
...
function mod:UpdatePartyUnit(unit)
	...
	if ( not UnitIsMappable(unit) and partyMembers[n] and not raidTargetsChanged ) then -- EDITED
		partyMembers[n] = free(partyMembers[n], self, n)
	elseif (raidTargetsChanged or ( UnitIsMappable(unit) and (not partyMembers[n] or partyMembers[n].freed ) ) ) then -- EDITED
		...
		if cl then
			...
			local unitTargetMarker = GetRaidTargetIndex(unit)
			if unitTargetMarker then
				guess = targetMarkerTable[unitTargetMarker] 
			else
			...
			-- How to clear the point before redrawing a new one?
			...
			raidTargetsChanged = false -- ADDED
		end
	end
end
...
function mod:OnInitialize()
	...
	local raidTargetsChanged = false; -- ADDED
end
...
function mod:OnEnable()
	...
	self:RegisterEvent("RAID_TARGET_UPDATE", "UpdateRaidTarget") -- ADDED
	self:UpdateParty()
end

-- ADDED --

function mod:UpdateRaidTarget()
	raidTargetsChanged = true;
	self:UpdateParty()
end
